...
@Name("promoted-video-item-full-bleed-fingerprint")
@MatchingMethod("Lkbq;", "<init>")
@GeneralAdsRemovalPatchCompatibility
@Version("0.0.1")
object PromotedVideoItemFullBleedFingerprint : MethodFingerprint(
    "V", AccessFlags.PUBLIC or AccessFlags.CONSTRUCTOR, listOf("L", "I"),
    customFingerprint = { methodDef ->
        methodDef.implementation?.instructions?.any { instruction ->
            (instruction as? WideLiteralInstruction)?.wideLiteral == GeneralAdsRemovalPatch.resourceIds[0]
        } == true
    }
)